...
- Reports imported bone-name compatibility as `xx of xx bones, xx.x% compatible` using only animated bones.
...
- `Check FBX Compatibility` only counts bones that actually have motion in the FBX action. Static bones are excluded from the compatibility percentage.
- You can also select the target rig as active plus one other armature in the scene, then use `Check Open Rig Compatibility`. This uses the comparison rig's active action and excludes bones that are not animated there.
